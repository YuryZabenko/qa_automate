...
export class LoginPage extends BasePage {
  constructor(page) {
    ...
    this.genders = [page.locator('#id_gender1'), page.locator('#id_gender2')];
  ...
  /**
   * Заполнение формы "Login to your account"  и нажатие на кнопку 'Login'
   * @param {boolean} valid - валидные данные
   * @returns {Promise<{email: string, password: string, name: string}|{name: *, email: string, password: *, firstName: *, lastName: *, company: *, address1: *, address2: *, state: *, city: *, zipcode: *, mobileNumber: *, gender: *, birthDate: *, birthMonth: *, birthYear: *, country: *}>}
   */
  async loginUser(valid) {
    return await test.step(`Login user, data valid: ${valid}`, async () => {
      let user;
      if (valid) {
      ...
      if (valid) {
        await this.header.logoutButton.waitFor({ state: 'visible' });
      }
      return user;
    });
  }
  ...
  async createAccountRandom() {
    return await test.step('Create account correct', async () => {
   ...
   * @param {boolean} valid - валидные данные
   * @return {Promise<{name: *, email: string, password: *, firstName: *, lastName: *, company: *, address1: *, address2: *, state: *, city: *, zipcode: *, mobileNumber: *, gender: *, birthDate: *, birthMonth: *, birthYear: *, country: *}|{email: string, password: string, name: string}>}
   */
  async signUpUser(valid) {
    return await test.step('Sign Up correct', async () => {
      let user;
      if (valid) {
        user = UserData.getRandomUser();
      } else {
